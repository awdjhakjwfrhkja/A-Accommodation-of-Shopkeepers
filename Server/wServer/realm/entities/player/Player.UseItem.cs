...
namespace wServer.realm.entities
{
    partial class Player
    {
            ...
            foreach (var eff in item.ActivateEffects)
            {
                switch (eff.Effect)
                {
                    case ActivateEffects.MysteryPortal:
                        ...
                        var descs = Manager.Resources.GameData.Portals.Where(_ => dungeons.Contains<string>(_.Value.ObjectId)).Select(_ => _.Value).ToArray();
                        ...
                        foreach (var player in Owner.Players.Values)
                            player.SendInfo(string.Format("{{\"key\":\"{{server.dungeon_unlocked_by}}\",\"tokens\":{{\"dungeon\":\"{0}\",\"name\":\"{1}\"}}}}", Name));
                        break;
                    case ActivateEffects.MysteryDyes:
                        {
                            if (item.ObjectId == "Mystery Stat Pot")
                            {
                                var itemList = new List<string> { "Potion of Defense (SB)", "Potion of Attack (SB)", "Potion of Speed (SB)", "Potion of Dexterity (SB)", "Potion of Vitality (SB)", "Potion of Wisdom (SB)", "Potion of Life (SB)", "Potion of Mana (SB)" }; //Have to provide a list of items, cannot be empty.

                                GiveRandomItem(itemList); //Gives the item.
                            }
                        }
                        break;

                    case ActivateEffects.MarblePillar:
                        {
                            Entity entity = Entity.Resolve(Manager, 0xb053);
                            entity.Move(X, Y);
                    ...
                    case ActivateEffects.HealingGrenade:
                        ...
                        break;
                    case ActivateEffects.DamageBlast:
                        AEDamageBlast(time, item, target, eff);
                        break;
                    case ActivateEffects.QuestChest:
                        AEQuestChest(time, item, target, eff);
                        break;
                    case ActivateEffects.EpicChest:
                        AEEpicChest(time, item, target, eff);
                        break;
                    case ActivateEffects.CharUnlocker:
                        AECharUnlocker(time, item, target, eff);
                        break;
                    case ActivateEffects.VaultUnlocker:
                        AEVaultUnlocker(time, item, target, eff);
                        break;
                    case ActivateEffects.FakeWhiteBag:
                        AEFakeWhiteBag(time, item, target, eff);
                        break;
                    default:
        ...
        public void GiveRandomItem(List<string> listOfItems)
        {
            Random ran = new Random();
            int chance = ran.Next(0, listOfItems.Count);
            for (var e = 4; e < Inventory.Length; e++)
            {
                ushort iten = Manager.Resources.GameData.IdToObjectType[listOfItems[chance]];
        ...
        private void AEQuestChest(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            if (item.ObjectId == "Quest Chest Item")
            {
                Entity entity = Entity.Resolve(Manager, 0xd8d); //"0x654" is the object ID of what you want to spawn when you consume item
        ...
        private void AEEpicChest(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            if (item.ObjectId == "Epic Quest Chest Item")
            {
                Entity entity = Entity.Resolve(Manager, 0xd8f); //"0x654" is the object ID of what you want to spawn when you consume item
                entity.Move(X, Y); //this spawns the item wherever you are
                Owner.EnterWorld(entity); //this makes it spawn basically in the world you are in
            }
        }

        private void AECharUnlocker(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            Client.Account.MaxCharSlot++;
            Client.Account.FlushAsync();
            Client.Player.SendInfo("New Character Slot unlocked successfully!");
        }

        private void AEVaultUnlocker(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            Client.Account.VaultCount++;
            Client.Account.FlushAsync();
            Client.Player.SendInfo("New Vault Chest unlocked successfully!");
        }

        private void AEFakeWhiteBag(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            if (item.ObjectId == "Trick White Bag")
            {
                Entity entity = Entity.Resolve(Manager, 0x1410); //"0x654" is the object ID of what you want to spawn when you consume item
                entity.Move(X, Y); //this spawns the item wherever you are
                Owner.EnterWorld(entity); //this makes it spawn basically in the world you are in
            }
        }
        ...
        private void AEDamageBlast(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
            var pkts = new List<Packet>
            {
                new ShowEffect()
